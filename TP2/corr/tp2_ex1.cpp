 ...
 * Retourne le nombre de maillons contenus dans la liste débutant par 'm'
 * Version récursive
 */
int taille_liste_rec_imp(maillon *m) {
	if(m == NULL) {
		return 0;
	} else {
		return 1 + taille_liste_rec_imp(m->suivant);
	}
}
 ...
 * O(n)
 */
int taille_liste_rec(liste l) {
	return taille_liste_rec_imp(l.tete);
}
...
int main() {
	...
	std::cout << "taille_liste_rec(l2) = " << taille_liste_rec(l2) << std::endl; // 4
